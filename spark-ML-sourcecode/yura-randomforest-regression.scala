// This is randomforest model for Yura's Cutting&Crimping Process
// created 2020 03 21
// -0.67
...
// target label is lable and setting MaxBins with 500 > max values of categorical variables
val rf = new RandomForestRegressor().setMaxBins(500).setLabelCol(lable).setFeaturesCol("Features")
...
val importances = model.stages.last.asInstanceOf[RandomForestRegressionModel].featureImportances
importances.toArray.zipWithIndex.map(_.swap).sortBy(-_._2).foreach(x => println(x._1 + " -> " + x._2))



// Select example rows to display.
//redictions.select("prediction", lable, "features").show(20)
...
val file = pmml.buildFile(new File("/Users/gary/Onedrive//rf.pmml"))

//spark-shell --jars ./jpmml-sparkml-1.5-SNAPSHOT.jar, ./jpmml-sparkml-executable-1.5-SNAPSHOT.jar
